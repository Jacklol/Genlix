...
В миграции включён RLS без разрешающих политик. Роли `anon` и `authenticated` не могут читать или менять CMS-таблицы. Сервер читает снимки и вызывает RPC с ролью `service_role`; прямые `UPDATE` и `DELETE` снимков, аудита и media metadata запрещены дополнительно триггерами.
...
4. В Table Editor должны появиться четыре таблицы, активная ревизия `0` и bucket `cms-media`.
